...
/* ============ 1) header.php: corregir ruta /assets/ si está rota ============ */
$headerFile = $base . '/app/Views/layouts/header.php';
$h = file_get_contents($headerFile);
if (preg_match('#href="/assets/css/#', $h)) {
    $h = str_replace(
        'href="/assets/css/horion-style.css"',
        'href="<?= $basePath ?? rtrim(dirname($_SERVER[\'SCRIPT_NAME\']), \'/\') ?>/assets/css/horion-style.css"',
        $h
    );
    file_put_contents($headerFile, $h);
    $report[] = 'header.php: ruta CSS absoluta CORREGIDA a ruta dinámica';
} else {
    $report[] = 'header.php: ruta CSS ya era correcta (sin cambios)';
}

/* ============ 2) footer.php: corregir ruta /assets/ si está rota ============ */
$footerFile = $base . '/app/Views/layouts/footer.php';
$f = file_get_contents($footerFile);
if (preg_match('#src="/assets/js/#', $f)) {
    $f = str_replace(
        'src="/assets/js/horion-app.js"',
        'src="<?= $basePath ?? rtrim(dirname($_SERVER[\'SCRIPT_NAME\']), \'/\') ?>/assets/js/horion-app.js"',
        $f
    );
    file_put_contents($footerFile, $f);
    $report[] = 'footer.php: ruta JS absoluta CORREGIDA a ruta dinámica';
} else {
    $report[] = 'footer.php: ruta JS ya era correcta (sin cambios)';
}

/* ============ 3) DashboardController: agregar marcarLeida() si falta ============ */
...
PHP;
    ...
    $report[] = 'DashboardController: método marcarLeida() AGREGADO';
} else {
    $report[] = 'DashboardController: marcarLeida() ya existía (sin cambios)';
}

/* ============ 4) CSS: parche de compatibilidad para vistas con <main> propio ============ */
...
if (strpos($css, 'COMPAT: vistas de módulos') === false) {
    ...
    $report[] = 'CSS: parche de compatibilidad AGREGADO';
} else {
    $report[] = 'CSS: parche ya estaba presente (sin cambios)';
}
...
if (!is_dir($up)) {
    mkdir($up, 0777, true);
    $report[] = 'Carpeta public/uploads/ CREADA';
} else {
    $report[] = 'Carpeta public/uploads/ ya existía';
}
?>
<!DOCTYPE html>
...
ul{font-size:.9rem;line-height:1.9}
...
<h1>🧩 Parches finales aplicados</h1>
<div class="big">✅ Proceso completado</div>
<div class="box"><ul>
<?php foreach ($report as $r): ?><li class="ok">✔ <?= htmlspecialchars($r) ?></li><?php endforeach; ?>
</ul></div>
<div class="box">
<h3>🧪 Recorrido de prueba (uno por uno):</h3>
