...
foreach ($it as $file) {
    ...
    // Calcular profundidad relativa desde la carpeta del archivo hasta app/Views
    $rel = str_replace('\\', '/', substr($fileDir, strlen($viewsDir)));
    $depth = ($rel === '' || $rel === '/') ? 0 : count(explode('/', trim($rel, '/')));
    $up = str_repeat('../', $depth); // ej: '../' para vistas en app/Views/modulo/

    $original = file_get_contents($path);

    // Reemplazar include/require relativos de layouts por versión con __DIR__
    ...
    if ($nuevo !== $original) {
        ...
        $fixed[] = str_replace(dirname(__DIR__) . '/', '', $path);
    } else {
        $skipped[] = str_replace(dirname(__DIR__) . '/', '', $path);
    }
}
?>
<!DOCTYPE html>
...
a{display:inline-block;margin-top:10px;color:#03a950;font-weight:600;background:#e8f5e9;padding:8px 14px;border-radius:8px;text-decoration:none}
</style></head><body>
<h1>🛠️ Reparador de includes en vistas</h1>
<div class="big">✅ <?= count($fixed) ?> vista(s) reparadas | <?= count($skipped) ?> ya estaban correctas</div>
<div class="box">
...
<?php else: ?><p>Ninguno (todos ya usaban __DIR__).</p><?php endif; ?>
</div>
<div class="box">
<h3>🧪 Prueba ahora:</h3>
